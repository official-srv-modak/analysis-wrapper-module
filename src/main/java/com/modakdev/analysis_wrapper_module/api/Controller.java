...
public class Controller {
    ...
    public Flux<String> chatSingleStream(@RequestBody Map<String, String> request) {
        ...
        return llmQueryService.getChatStream(query);
    }
    @PostMapping(value = "/chat-single-stream-sample", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<String> chatSingleStreamSample(@RequestBody Map<String, String> request) {
        String query = request.get("query");

        // Call the external service via WebClient and return the streaming response
        return llmQueryService.getChatStreamSample(query);
    }
}
